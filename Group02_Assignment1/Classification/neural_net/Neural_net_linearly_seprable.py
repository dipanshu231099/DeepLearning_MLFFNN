...
model.construct(data_x,data_y, [3,3], [logistic,logistic,logistic])
...
plt.legend()
plt.savefig("NN-Epoch vs average error graph LS")
plt.show()

true1,ouput1 = model.train_check()
true2,ouput2 = model.test_check()
...
accuracy(true1,ouput1)
accuracy(true2,ouput2)

class1 = []
class2 = []
class3 = []
xx =np.linspace(-15,15,num=500)
yy = np.linspace(-25,25,num=500)
for x in xx:
    for y in yy:
        tt = [x,y]
        tt = np.array(tt)
